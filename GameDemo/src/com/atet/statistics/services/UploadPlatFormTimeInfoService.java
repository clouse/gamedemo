...
public class UploadPlatFormTimeInfoService extends IntentService {
	...
	private void uploadInfos() {
		...
		List<InitInfo> list = PersistentSynUtils.getModelList(InitInfo.class, " autoIncrementId > 0");
		...
		if (result.getCode() == TaskResult.OK) {
			//上传成功后,删除数据库中的记录
			PersistentSynUtils.execDeleteData(InitInfo.class, " where autoIncrementId > 0");
