 ...
 */
public class BaseTask<T> extends AsyncTask<Void, Object, TaskResult<T>>
		implements Observer {
	/** 任务ID */
	 ...
	 * @param activity
	 *            任务依附的Activity实例
	 * @param taskPreListener
	 *            任务提交监听器实例
	 * @param taskPostListener
	 *            任务返回结果监听器实例
	 * @param taskKey
	 *            任务ID
	 */
	public BaseTask(Context context, AsynTaskListener<T> taskListener,
			Integer taskKey) {
	...
	protected void onPostExecute(TaskResult<T> result) {
		...
		if (isShowToast && result != null && result.getCode() != TaskResult.OK) {
