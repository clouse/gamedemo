...
public class FileUtils {
	...
	/**
	 * 
	 * @description: 替换文件的某一行 记录当前时间和userID
	 *
	 * @param content
	 * @param path
	 * @param append
	 ...
	 * @throws: 
	 * @author: LiJie
	 * @date: 2015年9月30日 上午11:17:49
	 */
	public static boolean writeToFile(String path, boolean append,String userID){
		boolean isNew = true;
		boolean res = false;
		...
		try {
			...
			if (f.canWrite()) {
				...
				//raf.seek(raf.length());
				if(temp.size() == 0){
					raf.writeBytes(userID + ":"+System.currentTimeMillis() + "\r\n");
				}else{
					for (int i = 0; i < temp.size(); i++) {
						String[] userInfo = temp.get(i).split(":");
						if(userID.equals(userInfo[0])){
							//发现相同的id，则替换
							raf.seek(i * (temp.get(i).getBytes().length + "\r\n".getBytes().length));
							raf.writeBytes(userID + ":"+System.currentTimeMillis() + "\r\n");
							isNew = false;
						}
					}
